...
uint8 FlashRead(uint32 ReadAddr, uint8* pBuffer, uint16 NumByteToRead)   
{ 
 	uint16 i;   										    
	SPI_CS_ENABLE;                            //使能器件   
...
//CHECK OK
uint8 FlashWrite(uint32 WriteAddr,uint8* pBuffer,uint16 NumByteToWrite)   
{ 			 		 
	uint16 pageremain;	   
...
//擦除一个山区的最少时间:150ms
uint8 FlashEraseSector(uint16 SectorNum)   
{
	uint32 Dst_Addr;
